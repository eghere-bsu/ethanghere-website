...
class Dao {
  ...
  public function createPost($user_id, $title, $description, $post_link, $image_url, $category) {
    ...
    // Prepare a query to insert a new post into the Posts table
    $createPostQuery = "INSERT INTO Posts (user_id, title, description, post_link, image_url, category)
                        VALUES (:user_id, :title, :description, :post_link, :image_url, :category)";
    $q = $conn->prepare($createPostQuery);
