$(document).ready(function () {
   ...
   $('.post-modal').click(function () {
      console.log("test");
      $('#post-modal').show(); // Display the new post modal
   });

   // Close modal
   $('.modal-close').click(function () {
      $('#login-signup-modal').hide(); // Hide both modals
      $('#post-modal').hide();
   });
   ...
   // Handle post from redirect
   if (window.location.href.indexOf('#post') !== -1) { // Check for #post in the URL
      $('#post-modal').show(); // Display the new post modal
   }
});
